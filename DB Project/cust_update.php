...
if(!isset($_SESSION["loggedin"]) || $_SESSION["loggedin"] !== true || ($_SESSION["title"] !== 'a' && $_SESSION["title"] !== 'l')){
...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($username_err) && empty($password_err) && empty($confirm_password_err) && empty($firstname_err) && empty($lastname_err) && empty($email_err)){
        ...
        $sql = "UPDATE customers SET username=?, password=?, firstname=?, lastname=?, email=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssssi", $param_username, $param_password, $param_firstname, $param_lastname, $param_email, $param_id);
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if ($_SESSION["title"] == 'a') {
                    ...
                    header("location: lib_customers.php");
                }
                exit();
            } else{
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_num_rows($result) == 1){
                    ...
                    header("location: cust_error.php");
                    exit();
                }
...
<body style="background-color:burlywood;">
    <div class="wrapper">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                <h2 class="mt-5 mb-3">Update Customer</h2>
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        ...
                        <?php 
                        if ($_SESSION["title"] == 'a') {
                            echo '<a href="admin_customers.php" class="btn btn-danger ml-2">Cancel</a>';
                        } else {
                            echo '<a href="lib_customers.php" class="btn btn-danger ml-2">Cancel</a>';
                        }
                        ?>
                    </form>
                </div>
